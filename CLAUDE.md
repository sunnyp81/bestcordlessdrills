...
Amazon product thumbnails wired 2026-07-11 into `SpecPanel.astro` (optional `image` prop) via `src/lib/affiliate.ts`'s `amazonImage(asin)`, sourced from build-time `src/data/amazon-images.json` (gitignored, regenerated by `scripts/amazon-image-sync.mjs` prebuild hook, vendored from `amazon-creators-sync`). Live on `/` (DeWalt, Bosch) and `/best-combi-drill/` (DeWalt, Makita); Ryobi has no SpecPanel on any page so its image never renders even though its ASIN is fetched. No ASIN or no synced image = no `<img>`, page renders exactly as before. Deploys need `CREATORS_CREDENTIAL_ID`/`CREATORS_CREDENTIAL_SECRET` in the Cloudflare Pages build env (direct-upload CF Pages project, local builds) or the prebuild step warns and skips images gracefully.
